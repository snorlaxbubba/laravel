<x-layout>
    <x-slot name="content">
        ...
        class="relative flex justify-center min-h-screen bg-[#fefae0] sm:items-center py-4 sm:pt-0">
            ...
            @if ($category)
            <h1 class="text-center font-bold text-xl mb-3 text-[#283618]">Edit Category: {{ $category->name }}</h1>
            <form method="POST" action="/admin/categories/{{ $category->id }}/edit">
                @method('PATCH')
            @else
                <h1 class="text-center font-bold text-xl mb-3 text-[#283618]">Create Category</h1>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" id="name" class="border border-gray-400 p-2 w-full" value="{{ old('name') ?? $category?->name }}">
                @error('title')
